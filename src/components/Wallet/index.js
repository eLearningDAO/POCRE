 ...
 * }}
 */
// eslint-disable-next-line unicorn/prevent-abbreviations, no-unused-vars
...
const generateScriptAddress = (state) => {
  ...
  // eslint-disable-next-line no-unused-vars
  const script = PlutusScript.from_bytes(Buffer.from(state.plutusScriptCborHex, 'hex'));
  ...
  const ScriptAddress = Address
    .from_bech32('addr_test1wpnlxv2xv9a9ucvnvzqakwepzl9ltx7jzgm53av2e9ncv4sysemm8');
  // eslint-disable-next-line no-console
...
// the button to connect a cardano wallet.
// has 3 states: no wallet found, 1 wallet found, multiple wallets found
function ButtonConnectWallet(state, pollWallets, setState, refreshData) {
  const { wallets, whichWalletSelected } = state;
...
function Wallet() {
  ...
  // const [whichWalletSelected, setWichWalletSelected] = useState();
   ...
   */
  // eslint-disable-next-line no-unused-vars
  const handleTabId = (tabId) => setState(
    (previous) => previous === { ...previous, selectedTabId: tabId },
  );
   ...
   */
  // eslint-disable-next-line no-unused-vars
  const handleWalletSelect = (object) => {
    const whichWalletSelected = object;
  ...
  useEffect(() => {
    // pollWallets();
    if (state.whichWalletSelected) {
      refreshData();
    }
  }, [state.whichWalletSelected]);
      ...
      <div className="wallet-div">
        <div className="inner-heading">
          ...
          {state.wallets?.length
            ? ButtonConnectWallet(state, pollWallets, setState, refreshData) : null}

...
export const updateLitigationById = catchAsync(async (req, res): Promise<void> => {
  ...
  if (req.body.invitations) await Promise.all(req.body.invitations.map((id: string) => getInvitationById(id))); // verify invitations, will throw an error if any invitation is not found
  if (req.body.decisions) await Promise.all(req.body.decisions.map((id: string) => getDecisionById(id))); // verify decisions, will throw an error if any decision is not found

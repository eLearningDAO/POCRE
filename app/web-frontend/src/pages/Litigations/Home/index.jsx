        ...
        {litigations?.[activeLitigation]?.length > 0 ? (
          <div className="grid-container">
            {litigations?.[activeLitigation]?.map((x, index) => {
              const votedInFav = x?.decisions?.find(
                (decision) => {
                  const makerMatchesUser = decision.maker_id === user.user_id;
                  const decisionStatus = decision.decision_status;
                  console.log({
                    activeLitigation,
                    lit: x.litigation_title,
                    decision,
                    user,
                    makerMatchesUser,
                    decisionStatus,
                  });
                  return decision.maker_id === user.user_id
